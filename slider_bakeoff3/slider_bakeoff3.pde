...
final int DPIofYourDeviceScreen = 88; //you will need to look up the DPI or PPI of your device to make sure you get the right scale!!
...
// Variables for Lilli's implementation
...
int startAlphaFromSlider(int sliderValue) {
  int slideIncrement = int(sizeOfInputArea / (26-windowSize));
  return(sliderValue / slideIncrement);
}

void setupSlider() {
  cp5 = new ControlP5(this);
  cp5.addSlider("sliderValue")
     .setPosition(200.0, 270.0)
     .setSize(int(sizeOfInputArea), 10)
     .setRange(0, sizeOfInputArea);
}

void drawSliderText() {
  float sliderVal = cp5.getController("sliderValue").getValue();
  startAlpha = startAlphaFromSlider(int(sliderVal));
  int xOffset = 220;
  fill(255);
  textFont(createFont("Arial", 12)); //set the font to arial 24
  for (int i = 0; i < windowSize; i++) {
     text(alphabet[startAlpha + i], xOffset + 15 * i, 210);
  }
}
